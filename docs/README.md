...
| [specs/ci.md](specs/ci.md) | Running unattended (GitHub Actions): the invocation shape, credentials from the job environment, and keeping the results tree alive between invocations on an ephemeral runner — the commit-back pattern and its alternatives (cross-cutting, no schema) |
...
| [implementation/project-structure.md](implementation/project-structure.md) | The **target folder and file structure** — every directory the repository grows into, what belongs in each, the naming conventions, and the ordered questions that decide where a new file goes. Draws the layout [stack.md §4](implementation/stack.md#4-repository-layout) decides |
