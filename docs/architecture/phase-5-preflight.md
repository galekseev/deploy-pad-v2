...
**The execution contract.** The engine runs the script as a subprocess through **Node's own TypeScript execution** — `node <check>.ts`, with no bundled TypeScript runtime and nothing required on the operator's `PATH` (type stripping is enabled by default since Node 23.6 and stable since 24.12; the engine's baseline is Node 24 — [stack.md](../implementation/stack.md#1-runtime-baseline-node-24-or-newer)). Two consequences for check authors. Scripts must use **erasable syntax only**: no `enum`, no `namespace` with runtime code, no parameter properties, no `import =` — Node strips types, it does not transform them, and Node 26 removed the transform escape hatch, so this is a permanent property rather than a current limitation. And the **checks shipped with the engine are published as compiled JavaScript**, because Node refuses to strip types from files inside `node_modules`; the contract is unchanged either way — the shipped checks are neither privileged nor exempt, they are simply built before publishing. Then: **exit code `0` is a pass, anything else is a fail**; stdout/stderr are captured, and the check's output *is* its report — whatever the script prints (conventionally to stderr, on failure) is surfaced verbatim as the check's message in the preflight report and the attempt record. There is no machine-readable result channel: a free-form third-party check produces free-form findings, so a structured contract would either be empty ceremony or a schema the engine cannot interpret anyway — one exit code plus one human-readable message covers what the report needs (`balance 0.03 ETH < required ~0.25 ETH for 4 deployments` is a printed line, not a data structure). Out-of-process for three reasons: the backstop limit is only enforceable across a process boundary (a hung in-process promise cannot be killed in Node), a crashing check cannot take the engine down with it, and — see below — the no-secrets rule becomes a property of serialization rather than of discipline.
