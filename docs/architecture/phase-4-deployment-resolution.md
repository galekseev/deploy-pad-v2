...
The verdict is read entirely from the results tree, which makes that tree the **cross-invocation contract**: everything this phase decides — and every freeze below — is a comparison against records a previous invocation wrote. Losing the tree corrupts nothing, but it makes this inspection blind, so the verdict comes out "fresh deployment" for work that was in fact half-finished. On a machine with a persistent workspace that never happens; on an ephemeral CI runner it is the default unless the tree is deliberately carried over, which is why [ci.md](../specs/ci.md) treats persistence as a correctness concern rather than an optimization.
