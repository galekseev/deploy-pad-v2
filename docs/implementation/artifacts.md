...
> **Status:** Draft, written before the first slice. **Normative until the code exists**: once `packages/engine/src/contracts/` is populated, the TypeScript types are the contract and this document becomes their readable index. When the two disagree after that point, the types win and this document needs correcting.
