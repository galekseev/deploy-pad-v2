...
**The invariant now has a claimant.** A read-only pipeline visualizer — a browser application that reads a config set and draws the resolved graph — is planned, which turns the paragraph above from a precaution into a dependency. A browser has no filesystem and no `process.env`, so a rule that reaches for either is a rule that consumer cannot run at all. The package itself still waits for the rules to exist (phase 3 is S4); what cannot wait is the discipline, because it is cheap while the code is being written and a refactor of the engine's most intricate logic afterwards.

**Which makes the environment an argument, not a lookup.** `${env.VAR}` resolves at the last of the ten resolution steps, and FR-REF-019 fixes *which* values are correct — the real process environment — without saying which line of code performs the read. So the read happens once, in `cross/env-file.ts`, which already owns the mount's `.env`, and the resulting map is passed into the gate that needs it. Three things follow: the resolution rules stay runnable off-node, a test supplies one variable with an object literal instead of mutating a global, and the functions that can touch a credential are exactly the ones with the environment in their signature — env vars being where the real secrets live, since a vault entry is a pointer to one. The rule is enforced rather than intended: `process.env` joins `console.*`, `process.stdout` and `process.exit` in the eslint bans, with `env-file.ts` the single narrowed exemption.

One portability question this leaves open, to be answered by the slice that hits it: phase 3's structural fingerprint needs a hash, and node's `node:crypto` has no browser twin with the same synchronous shape. Whether the fingerprint travels into the shared rules or stays engine-side is S4's call, not a reason to weaken the invariant everywhere else.

**Two future consumers, and they want different things** — worth separating now, because conflating them would put run-execution code into a package designed to be pure:

| Consumer | Wants | Where it would live |
|---|---|---|
| **Visual editor** — authoring configs | The *rules*, evaluated without side effects: method and key-slot precedence, mapping arity, salt derivation, namespace scoping | `packages/shared`, under the purity invariant above |
| **An embedding application** — launching deployments | The *pipeline*: construct a run context and a config source, execute, observe progress, read a typed outcome | Exported from `@deploy-pad/engine`; it needs the filesystem, subprocesses and the network by definition |

The second one already has a working, if argv-shaped, surface: `runCli(argv, io)` returns an `ExitCode` instead of calling `process.exit`, and its `CliIo` bag injects the console streams, the log-file opener, the clock, the cwd and the env-file loader — so a test (or any other caller) drives a whole invocation in-process with nothing hitting the real environment. That is the mechanical form of NFR-060, and the seams a published SDK would build on rather than replace: the run context ([phase 1](../architecture/phase-1-invocation.md#the-run-context-is-the-programmatic-boundary)) and the config source ([phase 2](../architecture/phase-2-load-validation.md#the-config-source-is-a-seam-too)). What is deliberately *not* decided is the published shape — see [§9](#9-open-questions).
...
- **What a published SDK surface looks like.** The layering is settled and the seams exist ([§4](#4-repository-layout), NFR-060); the *export list* is not, and it is the part that becomes a semver promise the moment it ships. Open: which entry points are public (a run-context builder, a config-source builder over the generated types, one execute call per command); how progress reaches a caller who has no console — the structured log record shape is the obvious candidate, but "observe a run" and "read a log" are not the same API; what a typed outcome carries beyond the exit code that `summary.json` does not already carry; and what a resume means when the configs never came from a file, given that the deployment record's plan reference and structural fingerprint assume a source that can be re-read. Tracked as OI-17 in [requirements.md](../requirements.md); the trigger is a real second caller, not the anticipation of one.
