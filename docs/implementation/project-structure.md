...
- **When `packages/shared` appears**, and what moves into it. The trigger — a second consumer — has now fired: a browser-side pipeline visualizer is planned ([stack.md §4](stack.md#4-repository-layout)). What stays open is the timing, because the rules it wants are phase 3's and arrive with S4, and the contents, because phase 3's fingerprint hash has no synchronous browser twin. Until the move, the purity invariant of §7 is the whole of the preparation — and `process.env` is now lint-enforced rather than left to discipline.
